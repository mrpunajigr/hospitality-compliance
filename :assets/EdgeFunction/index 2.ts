...
async function importPrivateKey(privateKey) {
  ...
  // Import the key
  return await crypto.subtle.importKey('pkcs8', keyBuffer.buffer, {
...
async function checkCompliance(deliveryRecordId, clientId, temperatures) {
  ...
  const { data: settings } = await supabase.from('compliance_settings').select('rules').eq('client_id', clientId).single();
...
async function createComplianceAlerts(deliveryRecordId, clientId, violations, supplierName) {
  ...
  const { error } = await supabase.from('compliance_alerts').insert(alerts);
  if (error) {
