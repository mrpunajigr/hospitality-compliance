...
import { classifyProductTemperatureRequirements, analyzeTemperatureCompliance, analyzeLineItemsCount } from './ProductClassificationEngine.ts';
/**
 * Fallback structure recognition using basic text processing
 */ async function fallbackStructureRecognition(imageBuffer) {
 ...
 */ async function fallbackEntityExtraction(imageBuffer, structure) {
 ...
 */ async function fallbackDataExtraction(text) {
 ...
 */ async function fallbackProductClassification(lineItems) {
 ...
 */ async function fallbackTemperatureAnalysis(temperatures) {
...
/**
 * Fallback validation
 */ async function fallbackValidation(enhancedData) {
 ...
 */ export async function processDocumentWithEnhancedAI(imageBuffer, processorId, accessToken) {
  ...
  try {
    ...
    try {
      productClassification = await classifyProductTemperatureRequirements(entityData.text, enhancedData.lineItems);
    } catch (error) {
    ...
    try {
      temperatureAnalysis = await analyzeTemperatureCompliance(enhancedData.temperatures, productClassification);
    } catch (error) {
 ...
 */ async function extractDocumentStructure(imageBuffer, processorId, accessToken) {
 ...
 */ async function extractBusinessEntities(imageBuffer, processorId, accessToken, structure) {
  const base64Image = btoa(String.fromCharCode(...new Uint8Array(imageBuffer)));
  const requestBody = {
    rawDocument: {
      content: base64Image,
 ...
 */ async function enhanceExtractedData(text, entities) {
  return {
 ...
 */ function extractEnhancedSupplierInfo(text, entities) {
  console.log('🔍 Starting enhanced supplier extraction...');
  console.log('📄 Available entities:', entities.map((e)=>({
      type: e.type,
      text: e.mentionText,
      confidence: e.confidence
    })));
  // Phase 1: Try Google Cloud AI entity extraction first
  const supplierEntity = entities.find((e)=>e.type === 'supplier_name' || e.type === 'company_name' || e.type === 'vendor_name' || e.mentionText && isLikelySupplierName(e.mentionText));
  ...
  if (supplierEntity && supplierEntity.confidence > 0.5) {
    ...
    return {
      value: cleanSupplierName(supplierEntity.mentionText),
      confidence: Math.max(0.3, supplierEntity.confidence - 0.2),
      boundingBox: supplierEntity.boundingBox,
 ...
 */ function extractSupplierFromDocumentStructure(text) {
 ...
 */ function extractSupplierFromContext(text) {
 ...
 */ function isCommonFalsePositive(name) {
  ...
  return falsePositives.some((fp)=>name.toLowerCase().includes(fp.toLowerCase()) || fp.toLowerCase().includes(name.toLowerCase()));
}
/**
 * Check if text is likely a supplier name based on characteristics
 */ function isLikelySupplierName(text) {
 ...
 */ function extractEnhancedDeliveryDate(text, entities) {
 ...
 */ function extractEnhancedHandwrittenNotes(text, entities) {
 ...
 */ function extractEnhancedInvoiceNumber(text, entities) {
 ...
 */ function extractEnhancedTemperatures(text, entities) {
  const temperatures = [];
 ...
 */ function extractEnhancedLineItems(text, entities) {
  ...
  for (const line of lines){
    ...
    if (trimmedLine.length < 5 || trimmedLine.match(/^(qty|quantity|item|description|price|total)/i) || trimmedLine.match(/^[0-9\s\$\.\-]+$/)) {
      continue;
    }
...
function detectTemperatureUnit(tempText) {
  return tempText.toLowerCase().includes('f') ? 'F' : 'C';
}
function evaluateTemperatureCompliance(temp) {
...
function getTemperatureContext(text, boundingBox) {
...
function extractUnitSize(description) {
...
function calculateEstimatedValue(lineItems) {
 ...
 */ async function validateAndScoreExtraction(data) {
  ...
  const validatedTemperatures = await validateTemperatureReadings(data.temperatures, data.productClassification);
 ...
 */ async function validateSupplierData(supplier) {
  let confidence = supplier.confidence;
  let value = supplier.value;
  console.log('🔍 Validating supplier data:', {
    value,
    confidence,
    method: supplier.extractionMethod
  });
 ...
 */ function isLegitimateBusinessName(name) {
 ...
 */ function isProperBusinessFormat(name) {
 ...
 */ function isJustLocationName(name) {
  const locationIndicators = [
 ...
 */ async function validateDeliveryDate(deliveryDate) {
...
/**
 * Enhanced temperature validation with product context
 */ async function validateTemperatureReadings(temperatures, productClassification) {
...
/**
 * Enhanced line item validation
 */ async function validateLineItems(lineItems, productClassification) {
...
function calculateEnhancedConfidence(scores) {
  ...
  let finalScore = scores.supplier * weights.supplier + scores.delivery * weights.delivery + scores.temperature * weights.temperature + scores.classification * weights.classification + scores.extraction * weights.extraction + scores.compliance * weights.compliance;
...
function calculateExtractionQuality(supplier, deliveryDate, temperatures, lineItems) {
...
function cleanSupplierName(name) {
  return name.replace(/[^\w\s&.,'-]/g, '') // Remove invalid characters
  .replace(/\s+/g, ' ') // Normalize whitespace
  .replace(/^\W+|\W+$/g, '') // Remove leading/trailing punctuation
  .trim();
}
function getExpectedTemperatureRange(classification) {
...
function isTemperatureInExpectedRange(temp, unit, range) {
  let tempInC = temp;
  if (unit === 'F') {
    tempInC = (temp - 32) * 5 / 9;
  }
...
function categorizeProduct(description, classification) {
...
function calculateOverallConfidence(scores) {
  ...
  return Number((scores.supplier * weights.supplier + scores.delivery * weights.delivery + scores.temperature * weights.temperature + scores.classification * weights.classification).toFixed(2));
}
