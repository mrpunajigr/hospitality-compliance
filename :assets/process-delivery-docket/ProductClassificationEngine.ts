 ...
 */ export function classifyProductTemperatureRequirements(extractedText, lineItems) {
  ...
  products.forEach((product)=>{
    ...
    const classifiedProduct = {
      ...
      temperatureRequirement: PRODUCT_KEYWORDS[classificationResult.category]?.temperature || PRODUCT_KEYWORDS.ambient.temperature,
 ...
 */ function classifyByPatterns(description) {
  const lowercaseDesc = description.toLowerCase();
  // Frozen patterns
 ...
 */ function calculateKeywordConfidence(keyword, description, matchType) {
 ...
 */ function calculateRiskLevel(category, confidence) {
 ...
 */ export function analyzeLineItemsCount(extractedText, lineItems) {
  let products = [];
  let extractionMethod = 'text_analysis';
 ...
 */ function normalizeProductName(name) {
  return name.toLowerCase().trim()// Remove quantities and sizes
  .replace(/\d+\s?(kg|g|ml|l|pack|box|dozen|each|ea)\b/gi, '')// Remove price indicators
  .replace(/\$[\d.,]+/g, '')// Remove common prefixes/suffixes
  .replace(/^(fresh|organic|free\s?range|premium|select)\s+/gi, '').replace(/\s+(fresh|organic|free\s?range|premium|select)$/gi, '')// Clean up whitespace
  .replace(/\s+/g, ' ').trim();
}
/**
 ...
 */ function extractProductsFromText(text) {
  ...
  for (const line of lines){
    ...
    if (trimmedLine.length < 3 || trimmedLine.match(/^[0-9\s\$\.\-]+$/) || trimmedLine.match(/^(total|subtotal|tax|gst|delivery|freight)/i)) {
      continue;
    }
 ...
 */ export function analyzeTemperatureCompliance(temperatures, classification) {
  const violations = [];
  ...
  for (const product of allProducts){
    const relevantTemps = temperatures.filter((temp)=>temp.productContext.toLowerCase().includes(product.name.toLowerCase()) || product.name.toLowerCase().includes(temp.productContext.toLowerCase()));
    ...
    for (const temp of tempsToCheck){
      ...
      if (tempValue < req.min || tempValue > req.max) {
        const severity = req.critical ? 'critical' : Math.abs(tempValue - (req.min + req.max) / 2) > 5 ? 'major' : 'minor';
        violations.push({
  ...
  const overallCompliance = violations.some((v)=>v.severity === 'critical') ? 'violation' : warningCount > 0 ? 'warning' : 'compliant';
  return {
    overallCompliance,
    violations,
    summary: {
