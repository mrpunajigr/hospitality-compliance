...
import { processDocumentWithEnhancedAI } from './EnhancedDocumentProcessor.ts';
...
async function processDeliveryDocket({ bucketId, fileName, filePath, userId, clientId }) {
  ...
  try {
    ...
    let enhancedExtraction;
    try {
      // Get access token for API calls
      const credentials = JSON.parse(GOOGLE_CREDENTIALS);
      const accessToken = await getAccessToken(credentials);
      // Process with enhanced multi-stage pipeline
      enhancedExtraction = await processDocumentWithEnhancedAI(imageBuffer, DOCUMENT_AI_PROCESSOR_ID, accessToken);
...
// =====================================================
async function validateUserAccess(userId, clientId) {
  try {
    const { data, error } = await supabase.from('client_users').select('id').eq('user_id', userId).eq('client_id', clientId).eq('status', 'active').single();
    return !error && !!data;
  } catch (error) {
...
async function importPrivateKey(privateKey) {
  try {
    ...
    const pemContents = cleanKey.replace(pemHeader, '').replace(pemFooter, '').replace(/\s/g, '') // Remove all whitespace including newlines
    ;
    ...
    const importedKey = await crypto.subtle.importKey('pkcs8', keyBuffer.buffer, {
...
function extractTemperatures(text) {
  ...
  // Remove duplicates
  return temperatures.filter((temp, index, self)=>index === self.findIndex((t)=>Math.abs(t.value - temp.value) < 0.1));
}
