...
export const registerDatabaseCore = async () => {
  try {
    ...
    // await registry.registerModule(dbModule) // Placeholder
    
    console.log('✅ Database Core module registered successfully')
    return true
    
  } catch (error) {
    console.error('❌ Failed to register Database Core module:', error)
    return false
  }
}
...
export const initializeDatabaseCore = async (config?: Record<string, any>) => {
  try {
    ...
    console.log('🚀 Database Core module initialized and active')
    return dbModule
    
  } catch (error) {
    console.error('❌ Failed to initialize Database Core module:', error)
    throw error
  }
}
