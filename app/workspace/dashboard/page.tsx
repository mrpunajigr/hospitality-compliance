...
export default function DashboardPage() {
  ...
  const handleDemoSignIn = async () => {
    try {
      // Try anonymous sign in first (if enabled)
      ...
      // If anonymous doesn't work, try with a simple email
      ...
      // Try creating with service role - this is a workaround for demo
      console.log('Attempting to create demo session...')
      
      // Create a temporary demo session using existing seeded user ID
      const demoUser = {
        id: 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a01', // Use existing user ID from seed data
      ...
      // For demo purposes, we'll simulate being signed in
      setUser(demoUser)
      
    } catch (err) {
      console.error('Demo sign in error:', err)
      alert('Demo authentication not available. Using offline demo mode.')
      
      // Fallback to demo user for UI purposes  
      const demoUser = {
        id: 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a01', // Use existing user ID from seed data
        email: 'demo@example.com',
        app_metadata: {},
  ...
  const handleUploadSuccess = (deliveryRecord: any) => {
    ...
    alert(`Successfully uploaded delivery docket! Processing started for record: ${deliveryRecord.id}`)
